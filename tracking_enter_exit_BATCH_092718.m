function tracking_enter_exit_BATCH_092718(loadconfig, pixpermm)
clc; close all;
warnid = 'images:initSize:adjustingMag';
warning('off',warnid);

timenow = datestr(now,'mm_dd_yy');
lawn_string = ['092718_BATCH_' timenow];
...
% in this version (092718), I directly supply the pixpermm value in order
% to accomodate with Shay's setups, which have a different pixpermm value
...
%         end
        ...
        rad_to_subtract = pixpermm/112*45; %NEW 09/27/18
...
for vid = 1:length(batch_struct)
    ...
    minblobarea = round((pixpermm/112)*300); % new addition 09/27/18
    ...
    [TRACKS, EXIT_STRUCT, POKE_STRUCT] = tracks_postprocessing3( allTracks, ev_ho_dict, bg_struct, pixpermm );
