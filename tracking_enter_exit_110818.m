...
function [TRACKS, EXIT_STRUCT, POKE_STRUCT, TRACKS_slim] = tracking_enter_exit_110818(loadconfig, pixpermm, showtracking_flag)
warnid = 'images:initSize:adjustingMag';
warning('off',warnid);

timenow = datestr(now,'mm_dd_yy');
lawn_string = ['110818_SOLO_' timenow];
maxwormnumber = 3;
...
while hasFrame(v2) || ~isempty(movienames) %MAIN LOOP
    ...
    for j = 1:size(bboxes,1)
        ...
        wormcrop_orig = imcomplement(imcrop(cropped,bbox_crop));
        cropworms_orig{j} = wormcrop_orig;
        %only calculate the spline if the centroid is a minimum distance away from the edge of the video
