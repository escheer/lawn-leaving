function [TRACKS, EXIT_STRUCT, POKE_STRUCT, SUMMARY_STRUCT] = tracks_postprocessing_resave( allTracks, ev_ho_dict, bg_struct, pixpermm, titlestr)
...
for i = 1:length(allTracks)
    ...
    angspeed = getAngularSpeed_NavinMethod(centroid_smooth); 
    TRACKS(outTrkIdx).angspeed = angspeed;
    
    %IDENTIFY HEAD AND TAIL, GET GRAYSCALE ASSOCIATED WITH THEM, THEN SMOOTH THEM, TOO
    TRACKS(outTrkIdx).head = allTracks(outTrkIdx).head;
    TRACKS(outTrkIdx).tail = allTracks(outTrkIdx).tail;
    TRACKS(outTrkIdx).head_gs = allTracks(outTrkIdx).head_gs;
    TRACKS(outTrkIdx).tail_gs = allTracks(outTrkIdx).tail_gs;
    TRACKS(outTrkIdx).omega = allTracks(outTrkIdx).omega;
    TRACKS(outTrkIdx).spline = allTracks(outTrkIdx).spline;
    TRACKS(outTrkIdx).curvature = allTracks(outTrkIdx).curvature;
    TRACKS(outTrkIdx).posture_angle = allTracks(outTrkIdx).posture_angle;
