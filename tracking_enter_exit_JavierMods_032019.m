...
function [TRACKS, EXIT_STRUCT, POKE_STRUCT, TRACKS_slim, SUMMARY_STRUCT, bg_struct] = tracking_enter_exit_JavierMods_032019(loadconfig, showtracking_flag, pixpermm, getpixelsize)
warnid = 'images:initSize:adjustingMag';
warning('off',warnid);

timenow = datestr(now,'mm_dd_yy');
lawn_string = ['032019_Javier_' timenow];
...
if loadconfig
    ...
    %     [ ev_ho_x, ev_ho_y, lawn_limit_line] = geteventhorizon010819(orig_background,outer_boundary_mask);
...
while hasFrame(v2) || ~isempty(movienames) %MAIN LOOP
    ...
    pixremoval_scale_factor = (50/96);
    thresh_cleaned = bwareaopen(thresh,round(pixremoval_scale_factor*pixpermm));
    BWdfill = imfill(thresh_cleaned, 'holes');
    se = strel('disk',7);
    BWdil = imdilate(BWdfill,se);
    BWfinal = imclearborder(BWdil, 4);
...
TRACKS = tracks_postprocessing_JavierMods_032019( allTracks, ev_ho_dict, bg_struct, pixpermm, [videoname(1:end-4) '_' lawn_string] );
%% SAVING!
...
save([videoname '_' lawn_string '_FINAL' '.mat'],'TRACKS_slim','bg_struct');
