function [worm,vWorm,errNum,errMsg] = ...
    segWorm_Elias(img, oImg, frame, isNormalized, verbose, varargin)
% orig = img;
...
% ELIAS REMOVED ERR #107 + 108 11/08/2017
%     if ~isempty(lfCBendI)
%         
...
%     end
    ...
    if hArea > 2.8 * tArea / bodyScale %ELIAS changed ratio to 2.8 from 2 on 11/08/2017
        errNum = 109
    ...
    % Note: the area of the head and tail should be roughly the same size.
    % A 2-fold difference is huge!
    elseif tArea > 2.8 * hArea / bodyScale %ELIAS changed ratio to 2.8 from 2 on 11/08/2017
        errNum = 110
...
% ELIAS REMOVED ERR #111 11/08/2017
