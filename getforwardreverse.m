function [head_vector, cent_vector, tail_vector, speed_smooth, moving_forward, moving_reverse] = getforwardreverse( cent,head,tail,speed,coherence_thresh,speed_thresh )
...
% cent_vector = [ c_smth2 - c_smth(1:end-int,:) ; zeros(int,2) ]; %add 0s at the end
% head_vector = [ h_smth2 - h_smth(1:end-int,:) ; zeros(int,2) ];
% tail_vector = [ t_smth2 - t_smth(1:end-int,:) ; zeros(int,2) ];
...
% h_t_vect_angles = zeros(size(cent_vector,1),1);
...
%     h_t_vect_angles(j) = atan2d(norm(cross(h,t)),dot(h,t));
...
% h_t_vect_angles = [zeros(int,1) ; h_t_vect_angles];
