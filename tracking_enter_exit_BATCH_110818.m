function tracking_enter_exit_BATCH_110818(loadconfig, tracklocally, pixpermm)
clc; close all;
warnid = 'images:initSize:adjustingMag';
warning('off',warnid);

timenow = datestr(now,'mm_dd_yy');
lawn_string = ['110818_BATCH_' timenow];
...
for vid = 1:length(batch_struct)
    ...
    while hasFrame(v2) || ~isempty(movienames) %MAIN LOOP
        if ~hasFrame(v2) %change the video here
            ...
            ev_ho_crp_rel = [ev_ho_x-region_rounded(1) ev_ho_y-region_rounded(2)]; %event horizon in cropped coordinates
            lawn_limit_mask_wf = poly2mask(lawn_limit_line(:,1),lawn_limit_line(:,2),size(filledbg,1),size(filledbg,2));
            fullyblurred_background = imgaussfilt(regionfill(filledbg,lawn_limit_mask_wf),4);
