function ALIGNED_DATA = align_data_to_exits( TRACKS, EXIT_STRUCT, before_sec, after_sec, stat_int )
%ALIGN_DATA_TO_EXITS.M This function takes in a set of tracks with pre-computed lawn-exits and entries,
% and comes up with a set of intervals around lawn-leaving events.
% stacks the following data around these events:
% speed, angular speed, roaming and dwelling, headpokes, head grayscale,
...
ALIGNED_EXIT_IND = before+1;
ALL_EXITS = EXIT_STRUCT.EXIT_TRKS;
%only take those exits that fall within the stat_int
acceptable_range_idx = EXIT_STRUCT.EXIT_FRAMES<=stat_int(2) & EXIT_STRUCT.EXIT_FRAMES>=stat_int(1);
ALL_EXITS = ALL_EXITS(acceptable_range_idx,:);
LL_OUT_INTS = EXIT_STRUCT.INTS_OUT_GLOBAL(acceptable_range_idx,:);
if ~isempty(LL_OUT_INTS)
    LL_DUR = LL_OUT_INTS(:,2)-LL_OUT_INTS(:,1); %in frames
else
    LL_DUR = [];
end

ALL_VIDEONAME = cell(size(ALL_EXITS,1),totallength);
ALL_VIDEOFRAME = zeros(size(ALL_EXITS,1),totallength);
ALL_FRAMESACTIVE = zeros(size(ALL_EXITS,1),totallength);
ALL_BGVIDINDEX = zeros(size(ALL_EXITS,1),totallength);
ALL_CENTX = zeros(size(ALL_EXITS,1),totallength);
ALL_CENTY = zeros(size(ALL_EXITS,1),totallength);
ALL_HEADX = zeros(size(ALL_EXITS,1),totallength);
ALL_HEADY = zeros(size(ALL_EXITS,1),totallength);
ALL_TAILX = zeros(size(ALL_EXITS,1),totallength);
ALL_TAILY = zeros(size(ALL_EXITS,1),totallength);
ALL_SPEED = zeros(size(ALL_EXITS,1),totallength);
ALL_ANGSPEED = zeros(size(ALL_EXITS,1),totallength);
ALL_ROAMINGDWELLINGHMM = zeros(size(ALL_EXITS,1),totallength);
ALL_HEADGS = zeros(size(ALL_EXITS,1),totallength);
ALL_EVHODIST = zeros(size(ALL_EXITS,1),totallength);
ALL_RADDIST = zeros(size(ALL_EXITS,1),totallength);
ALL_HEADPOKES = zeros(size(ALL_EXITS,1),totallength);
ALL_HEADPOKE_REVERSAL = zeros(size(ALL_EXITS,1),totallength);
ALL_LAWNENTRIES = zeros(size(ALL_EXITS,1),totallength);
ALL_LAWNEXITS = zeros(size(ALL_EXITS,1),totallength);
ALL_FORWARD = zeros(size(ALL_EXITS,1),totallength);
ALL_REVERSE = zeros(size(ALL_EXITS,1),totallength);
ALL_OMEGA = zeros(size(ALL_EXITS,1),totallength);
ALL_CENTROIDINLAWN = zeros(size(ALL_EXITS,1),totallength);
ALL_HEADINLAWN = zeros(size(ALL_EXITS,1),totallength);
ALL_TAILINLAWN = zeros(size(ALL_EXITS,1),totallength);
ALL_FULLYINLAWN = zeros(size(ALL_EXITS,1),totallength);

LL_INTS = cell(size(ALL_EXITS,1),2); %exit index, which track the exit came from, which interval of that track the exit came from


tracks_fa = {TRACKS(:).framesActive}';
for i = 1:size(ALL_EXITS,1)
    track_ind = ALL_EXITS(i,1);                     % the track index to which this lawn-leaving event belongs
    track = TRACKS(track_ind);                      % the actual track
    exit_ind = ALL_EXITS(i,2);                      % the index of the leaving event (in track indices)
    
    curr_int_global = [track.framesActive(exit_ind)-before track.framesActive(exit_ind)+after];
    ...
    trk_key_per_exit = NaN(size(curr_int_inds_global));             % for every index -- which track the data came from
    trk_inds_per_exit = NaN(size(curr_int_inds_global));            % for every index -- which track inds they are
    for j = 1:size(tracks_fa,1) %get the track indices across all tracks that match the framesActive that surround a lawn-leaving event
        ...
        if ~isempty(i_tfa)
            ...
            trk_key_per_exit(i_ciig) = j;
            trk_inds_per_exit(i_ciig) = i_tfa;
        end
    end
    LL_INTS{i,1}=trk_key;
    LL_INTS{i,2}=trk_ints;
    ...
    for k = 1:length(trk_key)
        ...
        if isfield(TRACKS, 'head_poke_reversal') %still missing this data for some 
...
ALIGNED_DATA = struct();
ALIGNED_DATA.LL_INTS = LL_INTS;
ALIGNED_DATA.LL_OUT_INTS = LL_OUT_INTS;
ALIGNED_DATA.LL_DUR = LL_DUR;
ALIGNED_DATA.ALIGNED_EXIT_IND = ALIGNED_EXIT_IND;
