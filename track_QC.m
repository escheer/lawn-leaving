...
foldername = out{end-1}; %can cause trouble for some long filenames
...
%     bg_struct = tracks.bg_struct;
    ...
    if sum(area>area_cutoff)~=length(area)
        ...
        if length(TRACKS_slim)>numtracks_cutoff %if you still have a ton of tracks after removing obvious crap, the whole thing is probably crap
            ...
            [ TRACKS_slim, FRAMES_IN_LAWN, ENTER_FRAMES, ENTER_TRKS, EXIT_FRAMES, EXIT_TRKS, EXIT_INTS, ALIGNED_EXIT_IND, EXIT_TRK_KEY, INTS_IN_BY_TRACK, INTS_OUT_BY_TRACK,INTS_IN_GLOBAL,INTS_OUT_GLOBAL, IN_INT_TRK_KEY, OUT_INT_TRK_KEY] = get_enter_exit_events2( TRACKS_slim, 45, 15 ); %get lawn entries and exits and intervals for each track surrounding the lawn exit event
            ...
            if exist('bg_struct','var')
                ...
                [TRACKS_slim, POKE_INTS_GLOBAL, POKE_INTS_BY_TRACK, POKE_TRK_KEY, POKE_PEAK_IDX_GLOBAL, POKE_PEAK_IDX_BY_TRACK, POKE_DIST_MINSUBTRACT, POKE_RAD_DIST, POKE_EVHO_DIST, POKE_GS] = get_head_pokes_noBGSTRUCT( TRACKS_slim, 0.970, INTS_OUT_BY_TRACK, OUT_INT_TRK_KEY );
            end
