function WORMTRACK = head_and_tail_kalman4(WORMTRACK)
...
tracks_combined = tracks_combined([tracks_combined(:).age]'>=5); %only tracks at least 5 frames long
...
%             disp(['other track ID is ' num2str(pID)]);
            ...
            if d2-d1>=5 % cent2 is the head
                ...
                for h = 1:size(cent2,1)
                    ...
                    [~,closeridx] = min(pdist([currspline(1,:);cent1(h,:);cent2(h,:)])); %if cent1 is the first spline point, closeridx will be 1, if cent2, then 2
                    if closeridx==1
